...
Endpoint parseEndpointInternal(const std::string& value)
{
...
Endpoint parseHostPortEndpointInternal(const std::string& value)
{
  ...
  if (util::startsWith(option, "p=")) {
    ...
    for (const auto& source : sources) {
      link.sources.push_back(
          parseEndpointInternal(std::string(source.first, source.second)));
    }
  }
}
...
{
  return parseHostPortEndpointInternal(value);
}
