...
void PortEventPoll::poll(const struct timeval& tv)
{
  ...
  if (res == 0 || (res == -1 && (errno == ETIME || errno == EINTR) &&
    ...
    for (uint_t i = 0; i < nget; ++i) {
      ...
      int errNum = errno;
      if (r == -1) {
        A2_LOG_DEBUG(fmt("port_associate failed for file descriptor %d:"
                        " cause %s",
    ...
    int errNum = errno;
    A2_LOG_DEBUG(fmt("port_getn error: %s", util::safeStrerror(errNum).c_str()));
  }
#ifdef ENABLE_ASYNC_DNS
